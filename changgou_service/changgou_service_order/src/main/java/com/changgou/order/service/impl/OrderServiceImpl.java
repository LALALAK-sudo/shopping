...
public class OrderServiceImpl implements OrderService {
    ...
    public String add(Order order){
        ...
        // 添加任务数据
        System.out.println("向订单数据库中的任务表去添加数据");
        Task task = new Task();
    ...
    public void closeOrder(String orderId) {
        // 1 根据订单id查询mysql中的订单状态
        // 2 基于微信查询订单信息
        // 2.1 如果当前订单的支付状态为已支付，则进行数据补偿
    ...
    public void confirmTask(String orderId, String operator) {
        ...
        order.setConsignStatus("2"); // 已经送达
        order.setOrderStatus("3"); // 已经完成
        order.setUpdateTime(new Date());
    ...
    public void autoTack() {
        // 1 从订单配置彪中，获得到订单自动确认的时间点
        // 2 得到当前时间节点向前数，（订单自动确认的时间节点）天，作为过期的时间节点
        // 3 从订单表中获得相关符合条件的数据（发货时间小于过期时间，收货状态为未确认）
        // 4 循环遍历，执行确认收货
